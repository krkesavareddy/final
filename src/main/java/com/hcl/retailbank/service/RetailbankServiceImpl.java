...
public class RetailbankServiceImpl implements RetailbankService {
	...
	public FundTransferResponce fundTransfer(FundTransferRequest fundTransferrequest) {
		...
		try {
			...
			CustomerCreation source = customerGetRepository
					.findByCustomerId(fundTransferrequest.getFromAccountNumber());
			...
			if (current > 5000 && update > 5000) {
				...
				stringBuilder
						.append("unable to process because insufficiends balance you must maintain minimum balance");
				fundTransferResponce.setResponceMessage(stringBuilder);
				return fundTransferResponce;
			}

			CustomerCreation destination = customerGetRepository
					.findByCustomerId(fundTransferrequest.getToAccountNumber());
			...
			fundTransferResponce
					.setResponceMessage(new StringBuilder("some internal problem please try after sometimes"));
			return fundTransferResponce;
